...
func saveBehaviors(name: String) {
    ...
    // 2
    let entity =
        NSEntityDescription.entity(forEntityName: "TargetBehaviors",
                                   in: managedContext)!
    
    let targetBehavior = NSManagedObject(entity: entity,
                                         insertInto: managedContext)
    
    // 3
    targetBehavior.setValue(name, forKeyPath: "name")
    ...
    do {
        ...
        targetBehaviors.append(targetBehavior)
...
func saveReplacementBehaviors(name: String) {
    ...
    // 2
    let entity =
        NSEntityDescription.entity(forEntityName: "ReplacementBehaviors",
                                   in: managedContext)!
    
    let replacementBehavior = NSManagedObject(entity: entity,
                                         insertInto: managedContext)
    
    // 3
    replacementBehavior.setValue(name, forKeyPath: "name")
    ...
    do {
        try managedContext.save()
        replacementBehaviors.append(replacementBehavior)
    } catch let error as NSError {
        print("Could not save. \(error), \(error.userInfo)")
    }
...
func setBehavior() {
        ...
        for i in 0..<targetBehaviorArray.count {
            saveBehaviors(name: "\(targetBehaviorArray[i])")
        }
    
        for i in 0..<replacementBehaviorArray.count {
            saveReplacementBehaviors(name: "\(replacementBehaviorArray[i])")
        }
    }
